    ...
    public class PontoTuristicoRepository : IPontoTuristicoRepository
    {
        ...
        public async Task<List<PontoTuristicoModel>> ObterTodosAsync()
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var query = " SELECT Ptu.IdPontoTuristico," +
                            ...
                            "        Ptu.Referencia," +
                            "        Ptu.Descricao," +
                            "            UfCodigoIbge = Ufe.CodigoIbge, " +
                            "        Ptu.InclusaoDataHora " +
                            "        FROM dbo.PontoTuristico    AS Ptu" +
                            "  INNER JOIN dbo.UnidadeFederativa AS Ufe ON Ptu.IdUf = Ufe.IdUf" +
        ...
        public async Task<List<PontoTuristicoModel>> CadastrarPontoTuristicoAsync(PontoTuristicoDto pontoTuristicoDto)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var queryCheckPontoTuristico = "SELECT TOP 1 1 " +
                                               "  FROM dbo.PontoTuristico AS Ptu " +
                                               " WHERE Ptu.Nome   = @Nome " +
                                               "   AND Ptu.IdUf   = @IdUf " +
                ...
                var queryInsertPontoTuristico = " INSERT INTO dbo.PontoTuristico( Nome, IdUf, Cidade, Referencia, Descricao, InclusaoDataHora ) " +
                                                " VALUES( @Nome, @IdUf, @Cidade, @Referencia, @Descricao, GETDATE() ); " +
